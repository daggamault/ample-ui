...
const stars = (filled: number, total = 5) =>
  Array.from(
    { length: total },
    (_, i) =>
      `<span class="rating-star${i < filled ? ' rating-star-filled' : ''}">${starSvg}</span>`
  ).join('');
    ...
    <div class="flex flex-col gap-10">
      ...
      ${section(
        ...
        `<div class="flex flex-col gap-3">
          <div class="rating rating-sm">${stars(3)}</div>
          <div class="rating">${stars(4)}</div>
          <div class="rating rating-lg">${stars(5)}</div>
        </div>`,
      ...
      ${section(
        ...
        `<div class="flex flex-col gap-2">
          ${[5, 4, 3, 2, 1]
            .map(
              (n) =>
                `<div class="flex items-center gap-3"><div class="rating rating-sm">${stars(n)}</div><span class="text-sm text-text-muted">${n}.0</span></div>`
            )
            .join('')}
        </div>`,
        code('<div class="rating rating-sm">\n  <span class="rating-star rating-star-filled"><svg>...</svg></span>\n  ...\n</div>')
      )}
    </div>`
};
